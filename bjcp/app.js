...
const Utils = {
  ...
  linkifyGlossary(text) {
    ...
    replacements.forEach(({ token, termIdx, original }) => {
      res = res.split(token).join(`<span class="glossary-link" onclick="App.showGlossaryPopup(event, ${termIdx}); event.stopPropagation();" title="Veure definició">${original}</span>`);
    });
  ...
  getGlossaryExamples(termObj) {
    ...
    return `<div style="margin-top:8px;font-size:0.85em;color:var(--text3);">
              <strong>Exemples on apareix:</strong> ${topExamples.join(', ')}
            </div>`;
  },
    ...
    Utils.el('modal-content').innerHTML = `
      ...
      <div class="modal-sections">
        ${sections.map(([title, text], i) => {
          ...
          } else if (title !== 'Comparació d\'estils') {
